...
import { getCourses } from "@/services/course.service"; //  Tera Course API Service
...
export function VideoFilters({
  ...
  const handleClear = () => {
    setLocalSearch("");
    onFilterChange({ search: "", courseId: "", sectionId: "", status: "" });
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111] p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row gap-3">
      {/* 🔍 Search Input for Video/Lesson Title */}
      <div className="relative flex-1">
        ...
        <input
          type="text"
          placeholder="Search videos by lesson title..."
          value={localSearch}
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
        ...
        {/*  Course Filter (Dynamic Search Select) */}
        <div className="w-full sm:w-64 shrink-0 z-50">
          <DynamicSearchSelect
            ...
            fetchFn={async (query) => {
              try {
                // Fetching courses via API based on user search query
                const res = await getCourses({ search: query, limit: 10 });
                // Check your backend response structure. Usually it's res.data or res.courses
